...
with sync_playwright() as p:
    ...
    check('A new discovery shows its Jev category','動物っぽい' in page.locator('.discovery-classification').inner_text())
    check('All seven perspectives and name caution are expanded by default',page.locator('.score-list .score-row').count()==7 and page.locator('.name-caution').is_visible() and page.locator('#verdict-stage details').count()==0)
    ...
    check('A discovery gains an imagined sound personality',page.locator('.discovery-classification .flavor-badge').inner_text()=='語感は、ふわふわ')
    page.locator('[data-verdict="next"]').click()
    ...
    page.evaluate("window.mockSpeech.onresult({results:[[{transcript:'モニュラッピ。'}]]}); window.mockSpeech.onend()")
    check('Speech transcript is editable and not auto-judged (ASR mock)',page.locator('#word-input').input_value()=='モニュラッピ' and not page.locator('#verdict-stage').is_visible())
    page.locator('#submit-button').click()
    ...
    start(page,shiri=True)
    page.locator('#mic-button').click(); page.locator('#mic-consent').click()
    page.evaluate("window.mockSpeech.onresult({results:[[{transcript:'雲ぷる餅。'}]]}); window.mockSpeech.onend()")
    ...
    check('Kanji speech reaches Jev unchanged',page.evaluate('window.mockCalls[0]')=={'word':'雲ぷる餅','reading':'','mode':'shiritori'})
    ...
    page = new_page(html=live_html,setup=combined_setup)
    start(page,shiri=True)
    page.locator('#word-input').fill('雲ぷる餅'); page.locator('#submit-button').click()
    ...
    page.locator('#reading-input').fill('クモプルモチ'); page.locator('#submit-button').click()
    ...
    for width in [320,390,768,1360]:
        ...
        page.evaluate(f"window.mockStatus='out'; window.mockAssessment={band_assessment}")
        start(page); answer(page,'サカナクション')
        check(f'Multi-angle scores are readable without horizontal overflow at {width}px',page.evaluate('document.documentElement.scrollWidth<=innerWidth') and page.locator('.judgment-panel').evaluate('(el)=>el.scrollWidth<=el.clientWidth'))
