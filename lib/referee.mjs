...
Optional visual ONLY for invented words: a nested visual object choosing shape, material, expression, detail from schema. This is a playful impression of the name, never evidence for existence. Omit visual when unsure; complete the verdict first.
Answer keys: kind, certainty, name, domain, detail, parts, issue, and optional visual. Return values, never a schema. Example for an invented sound: {"kind":"invented","certainty":"clear","name":"","domain":"other","detail":"新しく作った音の名前。","parts":[],"issue":"none"}`;

export function refereeSettings(env = {}) {
  const model = env.FALLBACK_MODEL ?? DEFAULT_REFEREE;
  return {model,enabled:env.FALLBACK_ENABLED !== 'false' && Object.hasOwn(REFEREE_MODELS,model),timeoutMs:6000};
}
...
export function parseRefereeResponse(data,input = {}) {
  ...
  if (value.visual && typeof value.visual === 'object' && !Array.isArray(value.visual)) {
    const SHAPES = ['blob','boulder','spark','ribbon','cloud','vessel','sprout','cluster'];
    ...
    const s = SHAPES.includes(value.visual.shape) ? value.visual.shape : null;
    const m = MATERIALS.includes(value.visual.material) ? value.visual.material : null;
    const e = EXPRESSIONS.includes(value.visual.expression) ? value.visual.expression : null;
    const d = DETAILS.includes(value.visual.detail) ? value.visual.detail : null;
    if (s || m || e || d) visual = {shape:s,material:m,expression:e,detail:d};
  }
  const res = {kind:value.kind,certainty:value.certainty,name:value.name.trim(),domain:value.domain,detail:value.kind === 'known' ? '既存のことば・名前としての手がかり。' : value.detail.trim(),parts:value.parts.map(p=>p.trim()),issue:value.issue};
  if (visual) res.visual = visual;
  return res;
}

export async function askReferee(input,{ai,model = DEFAULT_REFEREE,reserve,timeoutMs = 6000} = {}) {
