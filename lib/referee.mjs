...
export const REFEREE_VERSION = 'referee-v1';
export const DEFAULT_REFEREE = '@cf/qwen/qwen3-30b-a3b-fp8';
// Adapters share one verdict contract. Changing model never changes game rules.
export const REFEREE_MODELS = Object.freeze({
  [DEFAULT_REFEREE]: {label:'Qwen3 30B A3B',inputNeurons:4625,outputNeurons:30475},
  '@cf/google/gemma-4-26b-a4b-it': {label:'Gemma 4 26B A4B',inputNeurons:9091,outputNeurons:27273,chat_template_kwargs:{enable_thinking:false}}
});
...
export function parseRefereeResponse(data,input = {}) {
  ...
  if (value.kind === 'known' && (!value.name.trim() || !value.detail.trim())) throw new Error('Missing known-name evidence');
  if (value.kind === 'known' && [value.name,input.word].some(name=>name && C.toHiragana(name) === C.toHiragana(value.detail))) throw new Error('Repeated name is not evidence');
  if (['sentence','compound'].includes(value.kind) && !value.detail.trim()) throw new Error('Missing structure evidence');
...
export async function askReferee(input,{ai,model = DEFAULT_REFEREE,reserve,timeoutMs = 2000} = {}) {
  ...
  try {
    ...
    const work = async () => {
      ...
      return {state:'completed',answer:parseRefereeResponse(response,input)};
    };
