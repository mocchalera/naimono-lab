...
const PROMPTS = {
  ...
  invalid:'助っ人の答えがはっきりしなかったよ。今回はみんなで判定！',
...
export function resolveReferee(input,initial,fallback) {
  ...
  const visual = status === 'safe' ? (a.visual || initial.visual || null) : null;
  const result = {...base,status,decisionBy:'assistant',decisionReason:reason,message,visual};
...
export async function judgeGameWord(input,{ai,model,fallback = refereeSettings(),reserve,jevTimeoutMs} = {}) {
  const result = await judgeWord({...input, visual: true},{ai,model,timeoutMs:jevTimeoutMs, visual: true});
