...
export function buildRequest(word, reading = '', mode = 'free') {
  ...
  const questions = { [QUESTION_KEY]:EXISTENCE_QUESTION, ...DOMAIN_QUESTIONS, name_risk:NAME_RISK_QUESTION, category:CATEGORY_QUESTION, flavor:FLAVOR_QUESTION };
...
export function parseResponse(data, model = MODEL_ID) {
  ...
  let assessment;
  try { assessment = assessExistence(scores,risk?.type === 'score' && typeof risk.score === 'number' ? risk.score / 4 : null); }
